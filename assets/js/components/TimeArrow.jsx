...
export default class TimeArrow extends React.Component {
  ...
  render() {
    ...
    const path = `M${stroke},${(height - arrowHeight) / 2} 
      L${width - stroke - arrowPeekWidth},${(height - arrowHeight) / 2}
      L${width - stroke - arrowPeekWidth},${(height - arrowPeekHeight) / 2}
      L${width - stroke},${height / 2}
      L${width - stroke - arrowPeekWidth},${(height + arrowPeekHeight) / 2}
      L${width - stroke - arrowPeekWidth},${(height + arrowHeight) / 2}
      L${stroke},${(height + arrowHeight) / 2} 
      Z`;
    ...
    const gradLegends = arrayOfGradsToDisplay.map(([id, g]) => {
      ...
      return (
        <PosedLegend
          ...
          opacity={0.05}
          x={x}
          y={bounds.height - (bounds.height - arrowHeight) / 2 - 2}
          textAnchor="middle"
          fontFamily="Verdana"
          fontSize="10"
    ...
    const gradLines = arrayOfGradsToDisplay.map(([id, g]) => {
      ...
      const y2 = bounds.height - (bounds.height - arrowHeight) / 2 - 11;
        ...
        <g>
          ...
          {!isAnimating &&
            arrayOfGradsToDisplay.map(([id, g]) => {
              ...
              const beginY = (bounds.height - arrowHeight) / 2;
              return (
                <rect
