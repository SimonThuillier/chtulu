...
import HDateActionsView from './ui/hdateactionsview';
...
export default class HDateUI extends Plugin {
	...
	_createActionsView() {
		const editor = this.editor;
		const actionsView = new HDateActionsView( editor.locale );
		const hdateCommand = editor.commands.get( 'hdate' );
		// const unlinkCommand = editor.commands.get( 'unlink' );
        //
		actionsView.bind( 'data-hdate' ).to( hdateCommand, 'value' );
	...
	_addFormView() {
        ...
        this.formView.widget.showContainer();
		...
		//this.formView.urlInputView.inputView.element.value = hdateCommand.value || '';
	}
	 ...
	 */
	_closeFormView() {
		const hdateCommand = this.editor.commands.get( 'hdate' );

		// Restore manual decorator states to represent the current model state. This case is important to reset the switch buttons
		// when the user cancels the editing form.
        //hdateCommand.restoreManualDecoratorStates();

		if ( hdateCommand.value !== undefined ) {
	...
	_showUI( forceVisible = false ) {
		...
		if ( !this._getSelectedHDateElement() ) {
	...
	_startUpdatingUI() {
		...
		const update = () => {
			const selectedLink = this._getSelectedHDateElement();
			...
			//
			// Note: #_getSelectedHDateElement will return a link for a non-collapsed selection only
			// when fully selected.
