...
export const nodesCollide = function (node1,node2,margin=0,returnType=VERTICAL) {
    ...
    const L1R2 = node2Right - node1Left;
    const R1L2 = node2Left - node1Right;
    const hCollide = (((L1R2 > 0 && R1L2 > 0) || (L1R2 < 0 && R1L2 < 0))?-1:+1) * min(abs(L1R2),abs(R1L2));

    const B1T2 = node2Top - node1Bottom;
    const T1B2 = node2Bottom - node1Top;
    const vCollide = (((B1T2 > 0 && T1B2 > 0) || (B1T2 < 0 && T1B2 < 0))?-1:+1) * min(abs(B1T2),abs(T1B2));
    /*if((B1T2 > 0 && T1B2 > 0) || (B1T2 < 0 && T1B2 < 0)) return - min(abs(B1T2),abs(T1B2));
